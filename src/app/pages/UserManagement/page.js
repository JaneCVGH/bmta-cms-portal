...
>>>>>>> 01eeeed83ffe2e87e2e1204c0bc20ba13c357493
  ...
  useEffect(() => {
    ...
    apiRequest(`${API_V1}/department_command_stations`).then((res) => {
  ...
  const fetchRoles = async () => {
    const res = await apiRequest(`${API_V1}/role?start=0&length=100`, {
      method: "GET",
    });
  ...
  const pagedUsers = displayUsers.slice(startIndex, startIndex + rowsPerPage);
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        uuidFields.includes(name) && value === ""
  ...
  const handleOpenModal = async (type, user = null) => {
    ...
    setFormData({
      ...
      birthDate: userData?.bod ? userData.bod.split("T")[0] : undefined,
...
>>>>>>> 01eeeed83ffe2e87e2e1204c0bc20ba13c357493
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      const res = await apiRequest(`${API_V1}/users/${userId}`, {
        method: "DELETE",
      });
      if (!res.ok) {
      ...
      <div className={styles.container}>
        ...
        {viewMode === "table" ? (
          ...
          <table className={styles.tableEmployee}>
            <thead>
              <tr>
                <th className={styles.rowTable}>ผู้ใช้</th>
                <th className={styles.rowTable}>บทบาท</th>
                <th className={styles.rowTable}>แผนก</th>
                <th className={styles.rowTable}>สถานะ</th>
                <th className={styles.rowTable}>เข้าสู่ระบบครั้งล่าสุด</th>
                <th className={styles.rowTable}>การดำเนินการ</th>
              </tr>
            </thead>
                  ...
                  <div key={user.id} className={styles.userCard}>
                    ...
                    <div className={styles.userInfo}>
                      ...
                      <div className={styles.userStatus}>
                        <span className={styles.activeBadge}>✓ ใช้งานอยู่</span>
                      </div>
                    </div>
